...
public interface ExposedEngineRulesApi {

	@ApiOperation(value = "Returns a single rule within the exposed engine")
	@ApiResponses(value = {
	    @ApiResponse(code = 200, message = "Successful operation"),
	    @ApiResponse(code = 404, message = "Requested rule doesn't exist or the engine doesn't exist")
	})
	@RequestMapping(value = "/exposed-engine/rules/{ruleId}", method = RequestMethod.GET)
	ResponseEntity<?> getRule(
		@ApiParam(value = "",required=true) @PathVariable("ruleId") String ruleId
	);


	@ApiOperation(value = "Returns all rules within the exposed engine")
	@ApiResponses(value = {
	    @ApiResponse(code = 200, message = "Successful operation"),
	    @ApiResponse(code = 404, message = "Rule doesn't exist or the engine doesn't exist")
	})
	@RequestMapping(value = "/exposed-engine/rules", method = RequestMethod.GET)
	ResponseEntity<?> getRules();


	@ApiOperation(value = "Create a single rule within the exposed engine")
	@ApiResponses(value = {
	...
	@RequestMapping(value = "/exposed-engine/rules", method = RequestMethod.POST)
	ResponseEntity<?> createRule(
		@ApiParam(value = "" ,required=true ) @Valid @RequestBody Map<String, Object> rule
	);


	@ApiOperation(value = "Removes a single rule from the exposed engine")
	@ApiResponses(value = {
		@ApiResponse(code = 200, message = "Successful operation"),
		@ApiResponse(code = 404, message = "Requested rule doesn't exist within the given engine or the engine doesn't exist")
	})
	@RequestMapping(value = "/exposed-engine/rules/{ruleId}", method = RequestMethod.DELETE)
	ResponseEntity<?> deleteRule(
		@ApiParam(value = "",required=true) @PathVariable("ruleId") String ruleId
	);


	@ApiOperation(value = "Modify a single rule within the exposed engine")
	@ApiResponses(value = {
		@ApiResponse(code = 200, message = "Rule successfully modified"),
		@ApiResponse(code = 404, message = "Requested rule doesn't exist within the given engine or the engine doesn't exist")
	})
	@RequestMapping(value = "/exposed-engine/rules/{ruleId}", method = RequestMethod.PUT)
